-- Favorites & Quick Reorder System (FIXED VERSION)
...
-- Add search-related fields to menu_items (safe checks)
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_schema = 'public' 
                 AND table_name = 'menu_items' 
                 AND column_name = 'dietary_info') THEN
    ALTER TABLE public.menu_items ADD COLUMN dietary_info TEXT[] DEFAULT '{}';
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_schema = 'public' 
                 AND table_name = 'menu_items' 
                 AND column_name = 'allergens') THEN
    ALTER TABLE public.menu_items ADD COLUMN allergens TEXT[] DEFAULT '{}';
  END IF;
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
    ...
    ALTER TABLE public.menu_items ADD COLUMN spice_level INTEGER CHECK (spice_level >= 0 AND spice_level <= 5);
  END IF;
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_schema = 'public' 
                 AND table_name = 'menu_items' 
                 AND column_name = 'calories') THEN
    ALTER TABLE public.menu_items ADD COLUMN calories INTEGER;
  END IF;
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
    ...
    ALTER TABLE public.menu_items ADD COLUMN search_vector tsvector;
  END IF;
END $$;
